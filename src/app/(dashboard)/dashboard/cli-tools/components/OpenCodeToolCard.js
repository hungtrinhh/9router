...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl, canManageLocalSettings = true }) {
  ...
  const saveModels = async (models, newActiveModel, newSubagentModel) => {
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_9router" : selectedApiKey);
      const validActiveModel = newActiveModel !== undefined
        ? newActiveModel
        : (models.includes(activeModel) ? activeModel : (models[0] || ""));
      const validSubagent = newSubagentModel !== undefined ? newSubagentModel : subagentModel;
    ...
    <Card padding="xs" className="overflow-hidden">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                ...
                <div className="flex items-center gap-2 pl-9">
                  <BashSetupButton tool="opencode" baseUrl={getEffectiveBaseUrl()} apiKey={selectedApiKey} model={activeModel || selectedModels[0] || ""} models={selectedModels} subagentModel={subagentModel} className="!bg-yellow-500/20 !border-yellow-500/40 !text-yellow-700 dark:!text-yellow-300 hover:!bg-yellow-500/30" />
          ...
          {status && (
            <>
                          ...
                          <span
                            ...
                            onClick={() => {
                              if (model === activeModel) {
                                ...
                                saveModels(selectedModels, model);
                              }
                            }}
                          ...
                          >
                            ...
                            <button
                              onClick={(e) => {
                                ...
                                const nextActive = activeModel === model ? "" : activeModel;
                                setSelectedModels(newModels);
                                if (activeModel === model) {
                                  setActiveModel("");
                                }
                                saveModels(newModels, nextActive);
                              }}
                  ...
                  <div className="relative w-full min-w-0">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        debounceTimerRef.current = setTimeout(() => {
                          saveModels(selectedModels, activeModel, e.target.value);
                        }, 600);
                      }}
                    ...
                    {subagentModel && (
                      <button
                        onClick={() => {
                          setSubagentModel("");
                          saveModels(selectedModels, activeModel, "");
                        }}
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <BashSetupButton tool="opencode" baseUrl={getEffectiveBaseUrl()} apiKey={selectedApiKey} model={activeModel || selectedModels[0] || ""} models={selectedModels} subagentModel={subagentModel} variant="ghost" />
              </div>
            </>
          )}
        </div>
      ...
      {subagentModalOpen && (
        <ModelSelectModal
          ...
          onSelect={(model) => { setSubagentModel(model.value); setSubagentModalOpen(false); saveModels(selectedModels, activeModel, model.value); }}
