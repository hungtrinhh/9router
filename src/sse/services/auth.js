...
import { getCachedAntigravityQuota } from "./antigravityQuota.js";
  ...
  try {
    ...
    const availableConnections = connections.filter(c => {
      ...
      if (isAntigravity && model) {
        const quota = getCachedAntigravityQuota(c.id, model);
    ...
    if (availableConnections.length === 0) {
      ...
      if (isAntigravity && model) {
        connections.forEach((c) => {
          const resetAt = getCachedAntigravityQuota(c.id, model)?.resetAt;
